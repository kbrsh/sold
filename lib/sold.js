...
Sold.prototype.data = function(data) {
    ...
    this._blogAuthor = this._data.author;
...
Sold.prototype.build = function() {
    ...
    var blogAuthor = this._blogAuthor;
    ...
    fs.readdir(subSource, function(err, files) {
        ...
        files.forEach(function(file) {
            ...
            mkdirp(folderPath, function() {
                ncp(__dirname + "/../template/assets/", folderPath, function(err) {
                    if(err) {
                        console.log(colors.red("Sold ERR: Cannot copy assets => ") + err);
                    }
                });
                fs.writeFile(newPath, newHTML, function(err) {
                    if (err) {
                        console.log(colors.red("Sold ERR: Cannot write files => ") + err);
                    }
                });
            });            
        ...
        if(err) {
            console.log(colors.red("Sold ERR: Cannot read source directory => ") + err);
        }
    });
